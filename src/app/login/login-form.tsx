...
import { LogIn, Loader2, Eye, EyeOff, GraduationCap, ShieldCheck, Users, Shield, Globe } from "lucide-react";
...
export default function LoginForm({ tenantName, domain }: { tenantName: string, domain: string }) {
  ...
  const roleConfigs = {
    ...
    student: {
      ...
      desc: `Access your courses at ${tenantName}`
    },
    parent: {
      title: "Parent Portal",
      ...
      desc: `Monitor progress at ${tenantName}`
    }
  };
  ...
  async function onSubmit(data: LoginFormValues) {
    setLoading(true);
    try {
        const result = await loginAction(domain, data);

        if (result.success) {
            toast({ title: "Login Successful", description: `Welcome back to ${tenantName}!` });
            router.push(result.redirect || config.redirect);
        } else {
            toast({
                variant: "destructive",
                title: "Login Failed",
      ...
      <div className="bg-transparent border-none p-8 space-y-8">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
            {domain === 'default' && (
              <div className="bg-white/5 border border-white/20 p-4 rounded-2xl space-y-3 mb-6">
                <div className="flex items-center gap-2 text-white/70 text-xs font-bold uppercase tracking-widest">
                  <Globe className="h-3.5 w-3.5 text-brand-orange" /> Find Your Institution
                </div>
                <div className="flex gap-2">
                  <Input 
                    placeholder="e.g. soi, demo" 
                    value={tenantSearch}
                    onChange={(e) => setTenantSearch(e.target.value)}
                    className="h-10 bg-white/10 border-white/30 text-white placeholder:text-white/40 rounded-xl"
                  />
                  <Button 
                    type="button"
                    onClick={() => {
                        if (tenantSearch) {
                            ...
                            if (isVercel) {
                                window.location.href = `/login?tenant=${tenantSearch}`;
                                return;
                            }
                    ...
                    className="bg-brand-orange hover:bg-brand-orange/90 text-white h-10 px-4 rounded-xl font-bold"
                  >
                    Go
                  </Button>
                </div>
                <p className="text-[10px] text-white/40 italic">Enter your institution's unique identifier to be redirected to your secure portal.</p>
              </div>
            )}
        ...
        <div className="flex items-center justify-center gap-2 text-white/50 text-xs">
          ...
          <span className="font-bold tracking-widest uppercase italic">Encrypted Secure Session at {tenantName}</span>
        </div>
      </div>
    </div>
  );
