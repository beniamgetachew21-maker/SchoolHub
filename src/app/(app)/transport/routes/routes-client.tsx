...
export default function RoutesClient({ initialRoutes, vehicles, drivers, allStudents }: RoutesClientProps) {
  ...
  const handleAddRoute = async (data: any) => {
    // Implement addRouteAction later if needed, for now just toast and close
    toast({ title: "Note", description: "Route creation backend implementation pending." });
    setIsAddSheetOpen(false);
  }

  const handleUpdateRoute = async (data: any) => {
    toast({ title: "Note", description: "Route update backend implementation pending." });
    setIsEditSheetOpen(false);
  }

  const handleDeleteRoute = async () => {
    toast({ title: "Note", description: "Route deletion backend implementation pending." });
    setIsDeleteAlertOpen(false);
  }

  const handleManageStudents = async (route: any) => {
    setSelectedRoute(route);
    const students = await getStudentsOnRoute(route.routeId);
  ...
  const handleAssignStudent = async (studentId: string) => {
    if (selectedRoute) {
      ...
      try {
        const result = await assignStudentToRouteAction(studentId, selectedRoute.routeId);
        if (result.success) {
          const updatedStudents = await getStudentsOnRoute(selectedRoute.routeId);
          setRouteStudents(updatedStudents);
          toast({
          ...
          toast({
            title: "Error",
            description: result.error || "Failed to assign student",
            variant: "destructive",
          });
        }
      } finally {
