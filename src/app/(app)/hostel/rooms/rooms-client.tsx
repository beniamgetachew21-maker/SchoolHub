...
export default function RoomsClient({ initialRooms, hostels, students }: RoomsClientProps) {
  ...
  const handleAddRoom = (data: any) => {
    toast({ title: "Feature Coming Soon", description: "Room addition will be enabled in next patch." });
  ...
  const handleAllocateRoom = async (data: { studentId: string, roomId: string }) => {
    // Note: In real app, we need to pick a bed. For now, picking the first vacant bed.
      ...
      <CardContent>
        <Tabs defaultValue="all">
          ...
          <TabsContent value="all" className="mt-4">
            <RoomsTable rooms={filteredRooms} hostelMap={hostelMap} getStatusBadge={getStatusBadge} onDeallocateClick={setRoomToDeallocate} onAllocateClick={() => setIsAllocateSheetOpen(true)} getStudentById={getStudentById} />
          </TabsContent>
          <TabsContent value="vacant" className="mt-4">
            <RoomsTable rooms={filteredRooms.filter(r => r.status === 'Vacant')} hostelMap={hostelMap} getStatusBadge={getStatusBadge} onDeallocateClick={setRoomToDeallocate} onAllocateClick={() => setIsAllocateSheetOpen(true)} getStudentById={getStudentById} />
          </TabsContent>
          <TabsContent value="occupied" className="mt-4">
            <RoomsTable rooms={filteredRooms.filter(r => r.status === 'Occupied')} hostelMap={hostelMap} getStatusBadge={getStatusBadge} onDeallocateClick={setRoomToDeallocate} onAllocateClick={() => setIsAllocateSheetOpen(true)} getStudentById={getStudentById} />
          </TabsContent>
        </Tabs>
      </CardContent>
    </Card>
...
function RoomsTable({
  ...
  rooms: any[],
  hostelMap: Record<string, any>,
  getStatusBadge: (status: string) => JSX.Element,
  onDeallocateClick: (room: any) => void,
